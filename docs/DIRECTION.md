...
mtime만 믿으면 다운로드·압축 해제가 덮는다. 단어 수만 믿으면 초안이 더 길 수 있다.

- 파일 안 수정 시각 (파일시스템 mtime보다 우선)
- 본문 포함 관계
- 파일명 토큰 (`v3`, `최종`, `복사본`, `old`)
- OOXML revision / 저장 횟수
- 길이는 약한 신호

출력 예: `가족 #17, 후보 4개, 1위 이유: 본문이 나머지를 포함 + 내부 수정시각이 가장 늦음, 신뢰 0.72`.
